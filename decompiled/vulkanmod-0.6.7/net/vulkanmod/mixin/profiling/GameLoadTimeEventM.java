...
public class GameLoadTimeEventM {
   @Shadow
   @Final
   private Map<TelemetryProperty<Measurement>, Stopwatch> measurements;
   @Shadow
   @Final
   ...
   public void send(TelemetryEventSender telemetryEventSender) {
      Map<TelemetryProperty, Measurement> measurements = new Reference2ReferenceOpenHashMap();
      synchronized (this) {
         this.measurements
            .forEach(
               (telemetryProperty, stopwatch) -> {
                  if (!stopwatch.isRunning()) {
                     ...
                     measurements.put(telemetryProperty, new Measurement((int)l));
                  } else {
                     LOGGER.warn(
         ...
         this.bootstrapTime.ifPresent(l -> measurements.put(TelemetryProperty.LOAD_TIME_BOOTSTRAP_MS, new Measurement((int)l)));
      ...
      for (TelemetryProperty property : measurements.keySet()) {
         Measurement measurement = measurements.get(property);
